...
// TODO: Don't inherit from EventEmitter
...
DNSResolver.prototype.resolve4 = function(hostname, cb) {
  ...
  this._resolver.resolve4(hostname, function(err, addresses) {
    ...
    // It is possible that `dns.resolve4()` yields an empty `addresses` array.
    // This behavior has been observed when querying Consul's DNS server for an
    // external service in which the address is a hostname rather than an IP
    // address.  This seems to indicate that other records (including a CNAME)
    // exist for the queried hostname.
...
DNSResolver.prototype.resolveSrv = function(hostname, cb) {
  ...
  //
  //   $ dig @127.0.0.1 -p 8600 example.service.dc1.consul SRV
  var hn = service + '.service.' + labels.slice(2).join('.');
  this._resolver.resolveSrv(hn, function(err, addresses) {
    if (err) { return cb(err); }
    return cb(null, addresses);
  });
};
...
DNSResolver.prototype.resolveTxt = function(hostname, cb) {
  ...
  // TODO: Is it valid to query services for text records??
  if (service[0] == '_') {
    labels.splice(0, 2, service.slice(1), 'service');
  }
  
  // TODO: should this resolve in services, by default?  To match mdns?
  this._resolver.resolveTxt(labels.join('.'), function(err, records) {
    if (err) { return cb(err); }
