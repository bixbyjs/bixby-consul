...
HTTPUpdater.prototype.register = function(hostname, rrtype, record, cb) {
  ...
  switch (rrtype) {
    case 'A':
      return this.register4(hostname, record, cb);
    case 'CNAME':
      return this.registerCname(hostname, record, cb);
    case 'SRV':
      return this.registerSrv(hostname, record, cb);
  }
};

/**
 * Uses the Consul HTTP API to register a service record (`SRV` record) for the
 * `hostname`.  The `address` argument should be be an object with the following
 * properties:
 *
 * - `priority`
 * - `weight`
 ...
 *
 * @example
 * updater.registerSrv('_example._tcp.consul', { address: 'node1.test', port: 8080 }, function(err) {
 *   // ...
 * });
 */
HTTPUpdater.prototype.registerSrv = function(hostname, address, cb) {
  ...
  // To avoid these queries, an `address` should be set when using this updater
  ...
  // Internally, this makes a request to the [Agent HTTP API][1] to [register
  // a service][2].
  //
  // [1]: https://developer.hashicorp.com/consul/api-docs/agent
  // [2]: https://developer.hashicorp.com/consul/api-docs/agent/service#register-service
  this._client.agent.service.register(sdef, function(err) {
